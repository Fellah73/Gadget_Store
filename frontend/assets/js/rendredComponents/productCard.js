export function createProductCard(data) {
    ...
    card.className = "flex-shrink-0 w-64 md:w-72 bg-neutral-100 rounded-2xl shadow-md hover:shadow-gray-700 hover:shadow-xl transition-all duration-300 group animate-fade-in";
    ...
    card.innerHTML = `
        <div class="relative overflow-hidden rounded-xl">
            ...
            <div class="h-full bg-neutral-300 flex px-5 transition-transform duration-500 group-hover:scale-125">
                <img src="${data.image}" alt="${data.name}" class="h-full object-contain group-hover:animate-float" />
            </div>
        ...
        <div class="p-5">
            <!-- Category -->
            <p class="text-base text-gray-500 tracking-wider mb-1">${data.category}</p>
            ...
            <div class="flex items-center mb-2">
                ...
                <span class="text-sm text-gray-500 ml-2">(${data.reviews})</span>
            </div>

            <!-- Price -->
            <div class="flex items-center space-x-2 mb-3">
                <span class="text-lg font-bold text-primary-blue">${genreateNewProductCard(data)}€</span>
                ${data.oldPrice ? `<span class="text-sm text-gray-500 line-through">${data.oldPrice}€</span>` : ""}
            </div>
